...
- **三种摘要模式**：普通截断、全文直送、分段摘要，按需选择，灵活控制费用与质量。
...
- **分段摘要**：将全文按配置大小切块 → 各块独立提炼要点 → 二次汇总为完整摘要（Map-Reduce 模式）。
- **UI 交互**：通过 `addMenuItem` 动态注入 `zotero-itemmenu` 菜单，符合 XUL 规范。
- **网络层**：利用浏览器原生 `fetch` API，异步调用大模型接口，不阻塞主界面。
- **持久化**：使用 `new Zotero.Item('note')` 实现结果的结构化存储，便签标题注明摘要模式。
...
| :--- | :--- | :--- |
| **普通截断**（默认）| 快速预览，短文献 | 最低 |
| **全文模式** | 关闭截断，适合中等长度论文 | 中等 |
| **分段摘要** | 长篇论文，效果最佳 | 较高（多次请求）|
...
| 设置项 | 默认值 | 说明 |
| :--- | :--- | :--- |
| 选择模型 | DeepSeek | 切换 DeepSeek / Gemini |
| DeepSeek API Key | 空 | 在 platform.deepseek.com 获取 |
| Gemini API Key | 空 | 在 aistudio.google.com 获取 |
| 启用文本截断 | ✅ 开启 | 关闭后提取 PDF 全文 |
| 截断长度 | 10000 字符 | 截断时生效，可调至 200000 |
| 启用分段摘要 | ❌ 关闭 | 开启后忽略截断，全文分块处理 |
| 每段字符数 | 8000 字符 | 建议不超过模型单次上下文上限 |
...
- **分段摘要费用**：分段模式会对全文发起多次 API 请求，请注意 API 用量与费用。
